...
<body>
   ...
   <div style="margin: 20px;">
		...
		<!-- for fetching surveyor name from database -->
		...
		<% 
			...
			try{
				...
				PreparedStatement ps1 =con_capi.prepareStatement("SELECT EMP_ID, EMP_NAME FROM `EMP_MASTER` ORDER BY `EMP_ID` ASC");
				ResultSet rs1=ps1.executeQuery();
				while(rs1.next()){
					String sNumber=rs1.getString("EMP_ID");
					String sName=rs1.getString("EMP_NAME");
					//surveyorName.add(sName+" ("+sNumber+")");
					surveyorName.add(sNumber+"-"+sName);
				}
   ...
   //  System.out.println("select syr"+dept);
    ...
    <div class="container">
        <form action="dataInsert_STDP.jsp" method="post">
            <label for="surveyor">Select a Surveyor:</label>
            <select id="surveyor" name="surveyorName">
            
			<%
			    
			    for (String sValue:surveyorName) {
			%>
