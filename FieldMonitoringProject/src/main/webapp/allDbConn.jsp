...
<body>
	...
	String vidhansabhaId="";
		...
		try{
			...
			date_capi="%"+date_capi+"%";
			//System.out.println(date_capi);
	            ps_capi=con_capi.prepareStatement("SELECT * FROM `SAVE_SURVEY` WHERE `CHECKPOINT_ID` LIKE ? AND `VALUE` LIKE ? AND `DATETIME` LIKE ? ORDER BY `DATETIME` DESC");
		        ps_capi.setString(1,min_value);
		        ps_capi.setString(2,frName);
		      ps_capi.setString(3,date_capi);
		      
		    //  System.out.println(min_value+" "+frName+" "+date_capi);
	           rs_capi=ps_capi.executeQuery();
